<?php
    ...
    if ($check_subscribe) {
        $sql_insert = "DELETE FROM subscribes_on_users
                    WHERE (SELECT id FROM users WHERE username = '$subscriber_username') = subscriber_id
                      AND (SELECT id FROM users WHERE username = '$author_username') = author_id";
